...
export default function PayoffVisualizer() {
  ...
  const calculate = useCallback(async () => {
    const res = await api.calculatePayoff({
  ...
  const updateLeg = (i, field, value) => {
    const updated = [...legs]
    updated[i] = { ...updated[i], [field]: field === 'type' || field === 'position' ? value : parseFloat(value) || 0 }
    setLegs(updated)
  }
    ...
    <div>
      ...
      <div className="detail-section">
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
          {Object.keys(PRESETS).map(name => (
            <button key={name} className="btn btn-secondary" onClick={() => loadPreset(name)}>
              {name}
            </button>
          ))}
        </div>
      ...
      <div className="detail-section">
        ...
        <div className="viz-controls">
          <label>
            Current Stock Price ($)
            <input type="number" value={stockPrice} onChange={e => setStockPrice(parseFloat(e.target.value) || 0)} />
          </label>
        </div>
              ...
              {leg.type !== 'stock' && (
                <>
                  <label style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
                    K:
                    <input type="number" value={leg.strike} onChange={e => updateLeg(i, 'strike', e.target.value)} style={{ width: 80 }} />
                  </label>
                  <label style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
                    Premium:
                    <input type="number" value={leg.premium} onChange={e => updateLeg(i, 'premium', e.target.value)} style={{ width: 80 }} />
                  </label>
                </>
              )}
              <label style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
                Qty:
                <input type="number" value={leg.quantity} onChange={e => updateLeg(i, 'quantity', e.target.value)} style={{ width: 60 }} />
              </label>
              <button className="btn btn-secondary" onClick={() => removeLeg(i)} style={{ padding: '4px 10px' }}>×</button>
            </div>
          ))}
        </div>

        <div style={{ display: 'flex', gap: 8 }}>
          <button className="btn btn-secondary" onClick={addLeg}>+ Add Leg</button>
          <button className="btn btn-primary" onClick={calculate}>Calculate Payoff</button>
        </div>
      </div>
